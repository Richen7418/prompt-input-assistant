...
一个完全本地运行的桌面提示词助手，面向 Windows 10/11 与 macOS。第一版以 **ChatGPT Classic** 和新版 **ChatGPT** 桌面客户端为首批适配目标，也可以尝试在其他允许系统粘贴的普通输入框中使用。

它不是系统输入法，不会注册语言服务，也不会自动发送消息。输入半角分号 `;` 后，应用会打开自己的检索候选窗并接管中文输入法、上下方向键和 Tab；选中提示词后，应用恢复原窗口焦点并执行一次粘贴，然后重新显示候选窗以便继续选择。
